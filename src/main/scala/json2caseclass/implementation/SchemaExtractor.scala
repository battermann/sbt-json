...
object SchemaExtractor {
  ...
  private def extractSchemaFromJsObjectFields(
    ...
    for {
      ...
      fieldSchemas <- fields
        ...
        .map {
          case (fieldName, value) => extractSchemaFromJsValue(fieldName, value)
        }
        .sequence
      schema <- mkSchemaObject(
        env.nameTransformer.makeSafeCamelCaseClassName(name),
        fieldSchemas.map {
          case (n, v) => (env.nameTransformer.makeSafeFieldName(n), v)
        }
      )
    } yield schema
  }
  ...
  val exceptEmptyArrays: JsValueFilter => JsValueFilter = { include =>
    {
  ...
  val exceptNullValues: JsValueFilter => JsValueFilter = { include =>
    {
      case JsNull  => false
      case jsValue => include(jsValue)
    }
