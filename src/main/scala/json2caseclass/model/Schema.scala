...
object Schema {
  ...
  implicit class ToSchemaFieldName(name: String) {
    def toSchemaFieldName: SchemaFieldName =
      tag[SchemaFieldNameTag][String](name)
  }

  implicit class ToSchemaObjectName(name: String) {
    def toSchemaObjectName: SchemaObjectName =
      tag[SchemaObjectNameTag][String](name)
  }

  def haveSameStructure(a: Schema, b: Schema): Boolean = {
    (a, b) match {
      case (SchemaObject(_, _, fields1), SchemaObject(_, _, fields2)) =>
        fields1.size == fields2.size && fields1.forall {
          case (n1, e1) =>
            fields2.exists {
              case (n2, e2) => n1 == n2 && haveSameStructure(e1, e2)
            }
        }
