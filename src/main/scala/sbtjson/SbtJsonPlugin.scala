...
object SbtJsonPlugin extends AutoPlugin {
  ...
  private def generateCaseClassSourcesFromFiles(src: File,
                                                env: Config,
    ...
    sourceFiles.toList.map { file =>
      ...
      val name =
        NameTransformer.normalizeName("Model".toSuffix)(filename).toRootTypeName
      val json = Source.fromFile(srcFile).getLines.mkString
      CaseClassGenerator
        .generate(env)(json.toJsonString, name, getOptionals(optionals, name, packageName))
  ...
  private def generateCaseClassSourceFromUrls(urls: Seq[String],
                                              env: Config,
                                              optionals: Map[String, Seq[(ClassName, ClassFieldName)]],
                                              packageName: String): Either[SbtJsonFailure, List[(String, String)]] = {
    urls.toList.map { url =>
      Http
        ...
        .flatMap { json =>
          ...
          val name = NameTransformer
            .normalizeName("Model".toSuffix)(nameFromUrl)
            .toRootTypeName
          CaseClassGenerator
            .generate(env)(json.toJsonString, name, getOptionals(optionals, name, packageName))
  ...
  private def generateSourceFiles(src: File,
                                  dst: File,
                                  urls: Seq[String],
                                  env: Config,
                                  optionals: Map[String, Seq[(ClassName, ClassFieldName)]],
    ...
    for {
      ...
      generatedSources.map {
        case (fileName, generatedSource) =>
          val dstFile = packageName.split(".").foldLeft(dst) {
            case (acc, path) =>
              acc / path
  ...
  private def getOptionals(optionals: Map[String, Seq[(ClassName, ClassFieldName)]], name: String, packageName: String) = {
    optionals.getOrElse(name.toLowerCase, Nil) ++ optionals.getOrElse(s"$packageName.${name.toLowerCase}", Nil)
  }

  object autoImport {
    lazy val printJsonModels: TaskKey[Unit] =
      TaskKey[Unit]("print-json-models", "Prints the generated JSON models.")
    lazy val generateJsonModels: TaskKey[Seq[File]] = TaskKey[Seq[File]]("generate-json-models", "Generates JSON model case classes.")
    lazy val jsonInterpreter: SettingKey[Interpreter] =
      SettingKey[Interpreter]("json-interpreter", "Specifies which interpreter to use. `interpret` and `interpretWithPlayJsonFormats`")
    lazy val jsValueFilter: SettingKey[JsValueFilter] =
      SettingKey[JsValueFilter](
        "include",
    ...
    lazy val jsonSourcesDirectory: SettingKey[File] = SettingKey[File]("json-source-directory", "Path containing the `.json` files to analyze.")
    lazy val jsonUrls: SettingKey[Seq[String]]      = SettingKey[Seq[String]]("json-urls", "List of urls that serve JSON data to be analyzed.")
    lazy val jsonOptionals: SettingKey[Seq[OptionalField]] =
      SettingKey[Seq[OptionalField]](
        "json-optionals",
        "Specify which fields should be optional, e.g. `jsonOptionals := Seq(OptionalField(\"<package_name>\", \"<class_name>\", \"<field_name>\"))`"
      )
    lazy val packageNameForJsonModels: SettingKey[String] = SettingKey[String]("package-name-for-json-models", "Package name for the generated case classes.")
    lazy val scalaSourceDir: SettingKey[File] =
      SettingKey[File]("scala-source-dir", "Path for generated case classes.")
    ...
    val plainCaseClasses: Interpreter =
      CaseClassToStringInterpreter.plainCaseClasses
  ...
  private def toOptionalsMap(optionals: Seq[OptionalField]): Map[String, Seq[(ClassName, ClassFieldName)]] = {
    optionals
      ...
      .map {
        case (key, value) =>
          (key.toLowerCase, value.map {
            case OptionalField(_, cName, fName) =>
